...
namespace GuwbaPrimeAdventure.Item
{
	...
	internal sealed class InteractionRenderer : StateController, IImageComponents, IConnector
	{
		...
		{
			if ((this._isOnCollision = GuwbaAstral<CommandGuwba>.EqualObject(collision.gameObject)) && this._isActive)
				this._imagePool.Pull();
		}
		private void OnTriggerExit2D(Collider2D collision)
		{
			if (!(this._isOnCollision = !GuwbaAstral<CommandGuwba>.EqualObject(collision.gameObject)))
