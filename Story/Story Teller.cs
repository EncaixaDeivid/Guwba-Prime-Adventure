...
namespace GuwbaPrimeAdventure.Story
{
	...
	internal sealed class StoryTeller : MonoBehaviour
	{
		...
		{
			this._imageIndex = (ushort)(this._imageIndex < this._sceneObject.SceneComponents.Length - 1 ? this._imageIndex + 1 : 0);
			Texture2D texture = this._sceneObject.SceneComponents[this._imageIndex].Image;
		...
		internal IEnumerator NextSlide()
		{
			...
			while (this._sceneObject.SceneComponents[this._imageIndex].OffDialog)
			{
				yield return new WaitForSeconds(this._sceneObject.SceneComponents[this._imageIndex].TimeToDesapear);
				if (this._sceneObject.SceneComponents[this._imageIndex].JumpToNext)
